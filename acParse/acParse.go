...
func create() {
        requestURL := fmt.Sprintf("http://localhost:%d/bootaction/myfirstbootact                                                               ion", serverPort)
        kernelString := fmt.Sprintf("coreos/coreos/agent.x86_64-vmlinuz coreos.l                                                               ive.rootfs_url=http://%s:%s/coreos/agent.x86_64-rootfs.img ignition.firstboot ig                                                               nition.platform.id=metal",webServerHost,webServerPort)
